...
namespace FYFY_plugins.TriggerManager {
	...
	///     In this case the <see cref="FYFY_plugins.TriggerManager.Triggered3D"/> component (dynamically added) provides the list of <c>GameObject</c> in contact with him.
	/// </summary>
	...
	public class TriggerSensitive3D : MonoBehaviour {
		...
		private void OnTriggerEnter(Collider other){
			if (this.isActiveAndEnabled){
				...
				// If not, we have to use FYFY to add Triggered3D in order to keep families synchronized.
				...
				if (!GameObjectManager.containActionFor(typeof(UnbindGameObject), parents)){
					...
					if(_inCollision == false) {
						...
						GameObjectManager.addComponent<Triggered3D>(this.gameObject, true);
						_inCollision = true;
					}
				}
			}
		...
		private void removeTriggered(){
			...
			if(_targets.Count == 0 && _inCollision){
				...
				if (GameObjectManager.containActionFor(typeof(UnbindGameObject), parents)){
					...
					GameObjectManager.removeComponent<Triggered3D>(this.gameObject, true);
				}
			}
		}
